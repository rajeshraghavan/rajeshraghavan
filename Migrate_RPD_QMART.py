...
if argsLen -1 != 6:
   Log_file.write("The number of parameters passed to the RPD migration jython script is not 6. Please verify the number of parameters and run the script again")
   sys.exit("The number of parameters passed to the RPD migration jython script is not 6. Please verify the number of parameters and run the script again \n")

conncmd="connect(userConfigFile='//ziplock/obidev/" + bu_name + "/" + env_name + "/credentials/" + tgt_inst + "_configfile.secure',userKeyFile='//ziplock/obidev/" + bu_name + "/" + env_name + "/credentials/" + tgt_inst + "_keyfile.secure',url='" + WLS_URL + "')"
print conncmd
exec conncmd
...
try:
   params =  jarray.array([rpd_path,rpd_pwd],java.lang.Object)
...
#   invoke( 'uploadRepository', jarray.array([rpd_path,rpd_pwd],java.lang.Object), jarray.array(['java.lang.String', 'java.lang.String'],java.lang.String))
   ...
   print Date() , ': Unable to upload the rpd, releasing the lock now.'
   dumpStack()
   exit()
...
Log_file.write(str(Date()) + 'The service did not startup, look into this')
Log_file.close()
disconnect()
